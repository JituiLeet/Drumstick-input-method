    ...
    public String input(String s) {
        ...
        if (rime && session != 0) {
            ...
            for (int i = 0; i < lower.length(); i++) {
                ...
                if (c != null) handledAny = true;
                if (c != null && !c.isEmpty()) lastCommit = c;
            }
            // Some older/variant librime builds expose process_key but reject raw
            // printable keycodes. Use the documented simulate_key_sequence API as
            // a compatibility path rather than silently losing the keystroke.
            if (!handledAny && lower.length() > 0) {
                String c = RimeNative.simulateKeySequence(session, lower);
                if (c != null && !c.isEmpty()) lastCommit = c;
            }
            refresh();
            return lastCommit;
